...
# --- Константы для API proverkacheka.com ---
PROVERKACHEKA_HOST = "https://proverkacheka.com"
PROVERKACHEKA_ENDPOINT = "/api/v1/check/get"

# Токен доступа, который может быть задан в переменных окружения или использован встроенный
        ...
        try:
            ...
            if code == 1:
                return data # Успех
                 ...
                 # Чек еще обрабатывается ФНС - обычная ситуация при сканировании
                 # Вернем более информативное сообщение с возможностью повторить
                 raise Exception("Чек обрабатывается ФНС (обычно 1-2 мин). Нажмите 'Повторить' через минуту.")
            elif code == 3:
...
def parse_receipt_qr(qr_string: str) -> dict:
    ...
    try:
        ...
        # Конвертируем ISO формат в формат для datetime-local input
        if 'T' in date_formatted:
            date_formatted = date_formatted[:16]  # Оставляем только до минут
