...
def fetch_bitget_all_transactions(api_key: str, api_secret: str, passphrase: str = None, start_time_dt: datetime = None, end_time_dt: datetime = None, platform=None) -> dict:
    ...
    def _fetch_paginated_data_with_time(endpoint, id_key_for_record, pagination_param_name, base_params=None):
        ...
        # Only add time params if they're not already in current_params
        if start_ts_ms and 'startTime' not in current_params:
            current_params['startTime'] = start_ts_ms
        if end_ts_ms and 'endTime' not in current_params:
        ...
        while not stop_fetching:
            ...
            if last_id:
                ...
                # Удаляем временные параметры для последующих страниц
    ...
    # --- Deposits ---
    # Bitget requires startTime and endTime for deposits
    try:
        current_app.logger.info("\n--- [Bitget] Получение истории депозитов ---")
        deposit_params = {'limit': 100}
        if start_ts_ms:
            deposit_params['startTime'] = start_ts_ms
        if end_ts_ms:
            deposit_params['endTime'] = end_ts_ms
        all_txs['deposits'] = _fetch_paginated_data_with_time('/api/v2/spot/wallet/deposit-records', 'id', 'idLessThan', deposit_params)
    except Exception as e:
    ...
    try:
        ...
        withdraw_params = {'limit': 100}
        if start_ts_ms:
            withdraw_params['startTime'] = start_ts_ms
        if end_ts_ms:
            withdraw_params['endTime'] = end_ts_ms
        all_txs['withdrawals'] = _fetch_paginated_data_with_time('/api/v2/spot/wallet/withdrawal-records', 'withdrawId', 'idLessThan', withdraw_params)
    except Exception as e:
    ...
    try:
        ...
        # Используем tradeId для пагинации, предполагая, что Bitget использует idLessThan для этого эндпоинта.
